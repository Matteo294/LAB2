...
file_lettura = "../dati/contatti.csv" # Nome del file da cui caricare quelli esistenti.
file_scrittura = "../dati/contatti.csv" # Nome del file dove salvare tutti i dati
...
def plotxy(x , y):
    #print(type(y[-1]))
    plt.semilogx(x, y, 'go', linewidth=2, markersize=12, color=[0.01, 0.01, 0.01], alpha=0.5, markerfacecolor=[0, 0, 1])
    plt.grid()
    plt.xlabel("x")
    plt.ylabel("y")
    plt.title("X-Y Plot")
